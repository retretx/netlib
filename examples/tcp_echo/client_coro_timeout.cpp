...
task<std::string> echo_with_timeout(scheduler& sched, rrmode::netlib::net::event_loop& loop,
    ...
    co_await tcp_connect(client, loop, ep, sched, std::chrono::seconds{5}, &src);
    ...
    co_await sched;
    auto response =
        co_await read_string_with_timeout(client, loop, sched, std::chrono::seconds{5}, &src, 256);
...
int main(int argc, char** argv) {
    try {
        ...
        auto const response = sync_wait(
            sched, echo_with_timeout(sched, loop, {.host = "127.0.0.1", .port = port}, message));
...
int main() {
    std::cerr << "client_coro_timeout: только POSIX\n";
    return 1;
}
