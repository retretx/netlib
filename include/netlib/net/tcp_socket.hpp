...
/// TCP-сокет с неблокирующим I/O через event_loop.
class tcp_socket {
public:
    explicit tcp_socket(detail::socket_backend& sockets = detail::default_socket_backend())
        : sockets_{&sockets} {}

    tcp_socket(int fd, detail::socket_backend& sockets = detail::default_socket_backend())
        : sockets_{&sockets} {
        attach(fd);
    }

    /// Восстанавливает сокет по разделяемому handle (для цепочек async_*).
    explicit tcp_socket(std::shared_ptr<detail::socket_handle> handle) : handle_{std::move(handle)} {
    ...
    ~tcp_socket() { release_if_last_owner(); }
    ...
    /// Удерживает fd живым после уничтожения tcp_socket (пока идут async-операции).
    ...
    /// Снять ожидающую async-операцию (unregister fd + флаг отмены).
    ...
    /// Callback API (низкий уровень). Для coroutines: connect_async / tcp_connect.
    void async_connect(event_loop& loop, endpoint const& ep,
        ...
        int const fd = sockets_->create_tcp_socket();
        ...
        int const rc = sockets_->start_connect(h->native_handle(), ep);
        if (rc == 0) {
    ...
    /// Callback API. Для coroutines: read_some_async / read_exact_async.
    ...
    /// Callback API. Для coroutines: write_all_async.
